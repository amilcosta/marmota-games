...
export async function getPs5DTO(): Promise<GameDealDTO[]> {
    ...
    const {rows}  = await client.query('SELECT j."idJuego" as game_id,j.nombre as title, c.nombre as platform, j."idJuego" as deal_id, '+
        ...
        'and c."idConsola"=3 and c."idConsola" =pt."fkConsola" and pt.activo=1 and pt."fkConsola"=3 limit 5');
...
export async function getNewPs5DTO(): Promise<GameDealDTO[]> {
    const client = await pool.connect();
    const {rows}  = await client.query('SELECT j."idJuego" as game_id,j.nombrereal as title, c.nombre as platform, j."idJuego" as deal_id, j.nombre, '+
        ...
        'FROM "JUEGO" j, "CONSOLA" c, "DETALLEJUEGO" dj, "TIENDA" t, "PRECIOTIENDA" pt '+
        'WHERE j."idJuego"= dj."fkJuegoDetalle" and dj."fkConsolaDetalle"=c."idConsola" and j."idJuego"=pt."fkJuego" and pt."fkTienda"=t."idTienda" '+
        'and c."idConsola"=3 and pt.precio<=pt."precioViejo" and pt.activo=1 and pt."fkConsola"=3 '+ 
...
export async function getBestPs5DTO(): Promise<GameDealDTO[]> {
    const client = await pool.connect();
    const {rows}  = await client.query('SELECT j."idJuego" as game_id,j.nombre as title, c.nombre as platform, j."idJuego" as deal_id, '+
        'j.img_url as image_url, t."nombreTienda" as store_name, pt.precio as sale_price, pt."precioViejo" as original_price, '+
        'c."logoConsola" as logo_console, t."logoTienda" as logo_store, t."idTienda" as store_id '+
        'FROM "JUEGO" j, "CONSOLA" c, "DETALLEJUEGO" dj, "TIENDA" t, "PRECIOTIENDA" pt '+
        'WHERE j."idJuego"= dj."fkJuegoDetalle" and dj."fkConsolaDetalle"=c."idConsola" and j."idJuego"=pt."fkJuego" and pt."fkTienda"=t."idTienda" '+
        'and c."idConsola"=3 and pt.activo=1 and pt."fkConsola"=3 '+ 
...
export async function getOffertsPs5DTO(juegoId: Number): Promise<StoreDTO[]> {
    const client = await pool.connect();
    const query = 'SELECT t."idTienda" as store_id, t."logoTienda" as logo_store,j.nombre as title, t."nombreTienda" as store_name, '+
        ...
        'and j."idJuego"= dj."fkJuegoDetalle" and dj."fkConsolaDetalle"=c."idConsola" and c."idConsola"=3 '+
        'and pt.activo=1 and pt."fkConsola"=3 order by sale_price asc';
...
export async function getInfoGameStorePs5DTO(juegoId: Number): Promise<HistoryStore[]> {
    const client = await pool.connect();
    ...
    if(getmatch.rows[0].match!=null){
        for(const row of getmatch.rows){
            ...
            'and j."idJuego"=p."fkJuego" and p.activo=1 and p."fkConsola"=3 ';
            ...
            'WHERE ht."fkJuegoHistorial" = ANY($1) and t."idTienda"=ht."fkTiendaHistorial" and ht."fkConsola"=3 '+
        ...
        const query = 'SELECT ht.precio,ht.fecha,t."nombreTienda" as store_name, t."idTienda" as store_id '+
        'FROM "HISTORICOTIENDA" ht, "TIENDA" t '+
        'WHERE ht."fkJuegoHistorial"=$1 and t."idTienda"=ht."fkTiendaHistorial" and ht."fkConsola"=3 '+
        'order by fecha asc, store_name ';
...
export async function getInfoGamePs5DTO(juegoId: Number): Promise<GameInfo[]> {
    const client = await pool.connect();
    ...
    const queryInfo = 'SELECT j."idJuego" as game_id, j.nombre, dj."fechaLanzamiento" as release_date, dj.edicion, c."nombre" as platform, '+
        ...
        'WHERE j."idJuego"=$1 and j."idJuego"=dj."fkJuegoDetalle" and dj."fkConsolaDetalle"=c."idConsola" and c."idConsola"=3 ';
...
export async function getBestOffertsPs5DTO(juegoId: Number): Promise<StoreDTO[]> {
    const client = await pool.connect();
    const query = 'SELECT t."idTienda" as store_id, t."logoTienda" as logo_store,j.nombre as title, t."nombreTienda" as store_name, '+
        'pt.precio as sale_price, pt."precioViejo" as original_price, pt."enlaceTienda" as deal_url '+
        'FROM "JUEGO" j, "CONSOLA" c, "DETALLEJUEGO" dj, "TIENDA" t, "PRECIOTIENDA" pt '+
        'WHERE j."idJuego"=$1 and j."idJuego"=pt."fkJuego" and pt."fkTienda"=t."idTienda" '+
        'and j."idJuego"= dj."fkJuegoDetalle" and dj."fkConsolaDetalle"=c."idConsola" and c."idConsola"=3 and pt.activo=1 and pt."fkConsola"=3 '+
